...
const RAPIDAPI_KEY = "YOUR_RAPIDAPI_KEY"; // Replace with your WhatSMS RapidAPI key
...
function updateUIForRole() {
  ...
  document.getElementById(
    "current-user"
  ).textContent = `Logged in as ${currentUser.username} (${currentUser.role})`;
}

// Toggle Next Visit Fields
document
  .getElementById("schedule-visit-checkbox")
...
document.getElementById("signup-btn").addEventListener("click", async () => {
  ...
  if (existingUser) {
    document.getElementById("username-error").textContent = "Username exists";
    document.getElementById("username-error").style.display = "block";
    Swal.fire("Error", "Username exists!", "error");
    button.disabled = false;
    button.innerHTML = '<i class="fas fa-user-plus"></i> Signup';
    return;
  }
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (existingPatient) {
      ...
      Swal.fire(
        "Warning",
        `Patient with ID ${id} already exists: ${existingPatient.name}`,
        "warning"
      );
      return;
    }
    ...
    if (!pharmacy.trim()) {
      document.getElementById("pharmacy-error").textContent =
        "Pharmacy is required!";
      document.getElementById("pharmacy-error").style.display = "block";
      Swal.fire("Error", "Pharmacy cannot be empty!", "error");
      return;
    }
    if (age < 0 || age > 150) {
      document.getElementById("age-error").textContent =
        "Input correct age (0-150)";
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const pharmacy = document.getElementById("edit-pharmacy").value;
    ...
    if (!pharmacy.trim()) {
      document.getElementById("edit-pharmacy-error").textContent =
        "Pharmacy is required!";
      document.getElementById("edit-pharmacy-error").style.display = "block";
      Swal.fire("Error", "Pharmacy cannot be empty!", "error");
      return;
    }
    if (age < 0 || age > 150) {
      document.getElementById("edit-age-error").textContent =
        "Input correct age (0-150)";
    ...
    try {
      await fetch(`${API_URL}/patients/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, age, gender, contact, pharmacy }),
      });
      localStorage.setItem("patients", JSON.stringify(patients));
      renderPatients();
      e.target.reset();
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (!patient) {
      document.getElementById("examine-id-error").textContent =
    ...
    if (!notes.trim()) {
      document.getElementById("examine-notes-error").textContent =
    ...
    if (!nextStep) {
      document.getElementById("next-step-error").textContent =
    ...
    if (height <= 0) {
      ...
      Swal.fire("Error", "Height must be positive!", "error");
      return;
    }
    if (!/^\d+\/\d+$/.test(bp)) {
      document.getElementById("bp-error").textContent =
        "Use format systolic/diastolic (e.g., 120/80)";
    ...
    if (patient.status !== "Registered") {
      Swal.fire(
        "Error",
        `Patient status is ${patient.status}. Must be Registered.`,
        "error"
      );
      return;
    }
    ...
    try {
      ...
      Swal.fire(
        "Success",
        `Examination recorded for ${patient.name}!`,
        "success"
      ).then(() => {
      ...
      Swal.fire(
        "Error",
        "Failed to record examination. Saved locally.",
        "error"
      );
      localStorage.setItem("patients", JSON.stringify(patients));
      renderPatients();
...
document.getElementById("lab-form").addEventListener("submit", async (e) => {
  ...
  if (!patient) {
    document.getElementById("lab-id-error").textContent =
      "Patient ID not found!";
  ...
  if (!testType.trim()) {
    document.getElementById("lab-test-error").textContent =
  ...
  if (!results.trim()) {
    document.getElementById("lab-results-error").textContent =
  ...
  if (!diagnosis.trim()) {
    document.getElementById("lab-diagnosis-error").textContent =
      "Diagnosis is required!";
  ...
  if (patient.status !== "Examined") {
    Swal.fire(
      "Error",
      `Patient status is ${patient.status}. Must be Examined.`,
      "error"
    );
    return;
  }
  ...
  try {
    ...
    Swal.fire(
      "Success",
      `Lab results recorded for ${patient.name}!`,
      "success"
    ).then(() => {
...
document.getElementById("review-form").addEventListener("submit", async (e) => {
  ...
  const scheduleVisit = document.getElementById(
  ...
  if (!diagnosis.trim()) {
    document.getElementById("review-diagnosis-error").textContent =
      "Diagnosis is required!";
  ...
  if (!prescription.trim()) {
    document.getElementById("prescription-error").textContent =
  ...
  if (scheduleVisit) {
    if (!visitDate) {
      document.getElementById("next-visit-date-error").textContent =
    ...
    if (!reason.trim()) {
      document.getElementById("next-visit-reason-error").textContent =
        "Reason is required!";
      document.getElementById("next-visit-reason-error").style.display =
        "block";
  ...
  if (patient.status !== "Lab Processed" && patient.status !== "Examined") {
    Swal.fire(
      "Error",
      `Patient status is ${patient.status}. Must be Examined or Lab Processed.`,
      "error"
    );
    return;
  }
  ...
  if (scheduleVisit) {
    patient.nextVisit = { date: visitDate, reason, pharmacy: patient.pharmacy };
    patient.visits.push({
      date: new Date().toISOString().split("T")[0],
      stage: "Scheduled",
  ...
  try {
    ...
    Swal.fire(
      "Success",
      `Review recorded for ${patient.name}!`,
      "success"
    ).then(() => {
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const pharmacy = document.getElementById("dispense-pharmacy").value;
    const patient = patients.find((p) => p.id === id);

    if (!patient) {
      document.getElementById("dispense-id-error").textContent =
        "Patient ID not found!";
    ...
    if (!drugs.trim()) {
      document.getElementById("drugs-error").textContent =
        "Drugs are required!";
    ...
    if (!pharmacy.trim()) {
      document.getElementById("dispense-pharmacy-error").textContent =
        "Pharmacy is required!";
      document.getElementById("dispense-pharmacy-error").style.display =
        "block";
      Swal.fire("Error", "Pharmacy cannot be empty!", "error");
      return;
    }
    if (patient.status !== "Reviewed") {
      Swal.fire(
        "Error",
        `Patient status is ${patient.status}. Must be Reviewed.`,
        "error"
      );
      return;
    }
    ...
    try {
      ...
      Swal.fire(
        "Success",
        `Drugs dispensed for ${patient.name}!`,
        "success"
      ).then(() => {
...
// Set min date for next-visit-date input to today
document.addEventListener("DOMContentLoaded", function () {
  var nextVisitDate = document.getElementById("next-visit-date");
  if (nextVisitDate) {
    var today = new Date();
    ...
    nextVisitDate.setAttribute("min", minDate);
  }
});
...
function renderPatients(data = patients) {
  ...
  data.forEach((patient) => {
    ...
    const visitDate = patient.nextVisit
      ? new Date(patient.nextVisit.date)
      : null;
    const diffDays = visitDate
      ? Math.ceil((visitDate - today) / (1000 * 60 * 60 * 24))
      : null;
    ...
    const latestVisit =
      patient.visits.length > 0
        ...
        : null;
    const latestDiagnosis = latestVisit
      ? latestVisit.diagnosis || "None"
      : "None";
    const latestPharmacy = latestVisit
      ? latestVisit.pharmacy
      : patient.pharmacy;
    ...
    div.innerHTML = `
      ...
      <p>Pharmacy: ${latestPharmacy}</p>
      <p>Visits: ${patient.visits.length}</p>
      <p class="status">Status: ${status}</p>
      <p class="${isUrgent ? "urgent" : ""}">Next Visit: ${
      patient.nextVisit ? patient.nextVisit.date : "None"
    }</p>
      <p class="diagnosis">Latest Diagnosis: ${latestDiagnosis}</p>
      ${
        patient.nextVisit
          ? `<button class="sms-sent" onclick="sendNotification({id: '${patient.id}'})" aria-label="Send SMS reminder to ${patient.name}"><i class="fas fa-sms"></i> Send SMS Reminder</button>`
          : ""
      }
      ${
...
function editPatient(id) {
  ...
  document.getElementById("edit-pharmacy").value = patient.pharmacy;

  document.getElementById("patient-list").classList.add("hidden");
  document.getElementById("edit-patient").classList.remove("hidden");
}

// Send SMS Notification via WhatSMS
async function sendNotification(patient) {
  ...
  const message = `Dear ${patientData.name}, your next visit is on ${patientData.nextVisit.date} at ${patientData.nextVisit.pharmacy}. Reason: ${patientData.nextVisit.reason}. Contact: ${patientData.contact}`;

  const button = document.querySelector(
    `button[onclick="sendNotification({id: '${patient.id}'})"]`
  );
  if (button) {
    button.disabled = true;
  ...
  try {
    const response = await fetch("https://whatsms.p.rapidapi.com/send", {
      method: "POST",
      headers: {
